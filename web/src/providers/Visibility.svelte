<script lang="ts">
    import { ClientReceiveEvents, ClientSendEvents } from '$enums/events';
    import { VISIBLE, CONFIG } from '$stores/stores';
    import { ReceiveEvent, SendEvent } from '$utils/eventsHandlers';
    import { onMount } from 'svelte';

    ReceiveEvent(ClientReceiveEvents.showUI, (visible: boolean): void => {
        $VISIBLE = visible;
    });
    ...
    onMount(() => {
        ...
        const keyHandler = (e: KeyboardEvent) => {
            if ($VISIBLE && ['Escape'].includes(e.code)) {
                SendEvent(ClientSendEvents.hideUI);
            }
        };
