...
<div class="w-fit h-fit flex flex-col z-[9999999]">
    <button class="px-[1vw] py-[0.5vw] w-fit h-fit z-[9999999] bg-accent" on:click={() => (menuOpen = !menuOpen)}>
        Debug
    </button>

    {#if menuOpen}
        <ol class="flex flex-col gap-2 bg-primary z-[9999999] max-w-[25vw] h-full px-[0.5vw] py-[0.5vw]">
            {#each SendDebuggers as { label, actions }}
                <li class="flex flex-col gap-1 border-l-[2px] border-[color:var(--accent)] px-[0.25vw]">
